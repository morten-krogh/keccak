...
test("WASM NIST test 1", async (_t) => {
	...
	const result = await run_wasm_theta(h2b(remove_whitespace(hex_before_raw)));

	assert.equal(b2h(result), hex_after);
});

test("WASM NIST test 2", async (_t) => {
	const hex_before_raw = `
