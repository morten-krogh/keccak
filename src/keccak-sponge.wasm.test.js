 ...
 *   keccak_p: () => void,
 *   absorb: (c: number, m: number, should_pad: number) => void,
 ...
 * @param {boolean} should_pad
 * @returns {Promise<Uint8Array>}
 */
async function run_wasm_keccak(c_bytes, input, d_bytes, should_pad = true) {
	const exports = await instantiate_sponge();
	const memory = new Uint8Array(exports.memory.buffer);
	memory.fill(0);
	memory.set(input, DATA_OFFSET);
	exports.absorb(c_bytes, input.length, should_pad ? 1 : 0);
...
test("WASM keccak sponge empty input", async (_t) => {
	...
	assert.deepEqual(
		await run_wasm_keccak(64, input, 32),
...
test("WASM keccak sponge partial input", async (_t) => {
	...
	assert.deepEqual(
		await run_wasm_keccak(64, input, 64),
...
test("WASM keccak sponge multi-block squeeze", async (_t) => {
	...
	assert.deepEqual(
		await run_wasm_keccak(64, input, 200),
...
test("WASM keccak sponge sha3-512 1600-bit suffixed input", async (_t) => {
	...
	assert.deepEqual(
		await run_wasm_keccak(128, input, 64),
		run_js_keccak(128, input, 64),
	);
});

test("WASM keccak sponge unpadded pre-padded block", async (_t) => {
	...
	assert.deepEqual(
		await run_wasm_keccak(64, padded_input, 32, false),
		run_js_keccak(64, suffixed_input, 32),
	);
});

test("WASM keccak sponge traps on unpadded nonmultiple input", async (_t) => {
	await assert.rejects(
		async () => run_wasm_keccak(64, Uint8Array.from([1]), 32, false),
