...
const BENCH_PATH = fileURLToPath(
	new URL("./bench-sha3-256.js", import.meta.url),
);
const SHA3_256_A3X4 =
...
test("bench-sha3-256.js runs a small SHA3-256 benchmark", async (_t) => {
...
test("bench-sha3-256.js supports equals-form arguments", async (_t) => {
...
test("bench-sha3-256.js rejects unknown options", async (_t) => {
	const result = await run_bench([
		"--algorithm",
		"sha3-256",
		"--message-size",
...
test("bench-sha3-256.js rejects missing required arguments", async (_t) => {
...
test("bench-sha3-256.js rejects negative message sizes", async (_t) => {
...
test("bench-sha3-256.js rejects zero iterations", async (_t) => {
...
test("bench-sha3-256.js rejects negative warmup iterations", async (_t) => {
	const result = await run_bench([
		"--message-size",
		"4",
		"--iterations",
