...
const USAGE = `Usage: node src_wasm/bench.js --algorithm <${SUPPORTED_ALGORITHMS.join("|")}> --message-size <bytes> --iterations <count> [--warmup-iterations <count>]`;
...
function parse_args(args) {
	...
	const algorithm = flags.get("algorithm");
	if (algorithm === undefined) {
		usage_error("Missing required option --algorithm");
	}
	if (!SUPPORTED_ALGORITHMS.includes(algorithm)) {
		usage_error(`Unsupported algorithm: ${algorithm}`);
	}
...
async function run_bench(options) {
	...
	const sha3 = await new Sha3().initialize(wasm_module);
	...
	for (let iteration = 0; iteration < options.warmupIterations; iteration++) {
		sha3.createHash(options.algorithm).update(message).digest();
	}

	const start = performance.now();
	for (let iteration = 0; iteration < options.iterations; iteration++) {
		digest = sha3.createHash(options.algorithm).update(message).digest();
	}
