...
const wasm_bytes = await readFile(new URL("./chi.wasm", import.meta.url));
const chi_wasm_module = await WebAssembly.compile(wasm_bytes);
 ...
 */
function test_wasm_chi_vector(name, vectors_by_id, input_id, expected_id) {
	test(name, async (_t) => {
		const result = await run_wasm_chi(
...
test("WASM zero", async (_t) => {
	...
	const result = await run_wasm_chi(state_array);
...
test("WASM some ones", async (_t) => {
	...
	assert(!get_bit_in_state_array(result, 2, 0, 0));
	assert(get_bit_in_state_array(result, 3, 0, 0));
	assert(!get_bit_in_state_array(result, 4, 0, 0));
	assert(!get_bit_in_state_array(result, 0, 0, 5));
	assert(get_bit_in_state_array(result, 1, 0, 5));
	assert(get_bit_in_state_array(result, 2, 0, 5));
	assert(!get_bit_in_state_array(result, 3, 0, 5));
	assert(get_bit_in_state_array(result, 4, 0, 5));
	assert(get_bit_in_state_array(result, 0, 4, 23));
	assert(!get_bit_in_state_array(result, 1, 4, 23));
	assert(get_bit_in_state_array(result, 2, 4, 23));
	assert(get_bit_in_state_array(result, 3, 4, 23));
	assert(get_bit_in_state_array(result, 4, 4, 23));
});
...
for (let round = 0; round < 24; round++) {
	test_wasm_chi_vector(
		`WASM chi sha3-256_0 round ${round}`,
...
for (let absorb = 0; absorb < 3; absorb++) {
	for (let round = 0; round < 24; round++) {
		test_wasm_chi_vector(
			`WASM chi sha3-512_1600 absorb ${absorb} round ${round}`,
